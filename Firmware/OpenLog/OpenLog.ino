/*
2020
Sergey Stepanyuk: just changed the default baudrate to 38400
*/
...
/*
  ...
  OpenLog runs at 38400bps by default. This is configurable to 1200, 2400, 4800, 9600, 19200, , 57600, and 115200bps. You can alter
  ...
  see a message 'Going to 38400bps...' or some such message. You will need to power down OpenLog, change
  ...
  in unison. Now power down OpenLog and remove the RX/GND jumper. OpenLog is now reset to 38400bps.
...
long setting_uart_speed; //This is the baud rate that the system runs at, default is 38400. Can be 1,200 to 1,000,000
...
void checkEmergencyReset(void)
{
  ...
  //Now sit in forever loop indicating system is now at 38400bps
  while (1)
  {
    delay(500);
    toggleLED(stat1); //Blink the stat LEDs
...
void readSystemSettings(void)
{
  ...
  {
    setting_uart_speed = 38400; //Reset UART to 38400 if there is no speed stored
...
void readConfigFile(void)
{
  ...
  for (i = 0 ; i < len; i++)
  {
    ...
    if (settingNumber == 0) //Baud rate
    {
      ...
      if (new_system_baud < BAUD_MIN || new_system_baud > BAUD_MAX) new_system_baud = 38400; //Default to 38400
    }
